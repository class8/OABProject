	...
	<div class="main_content">
		...
		<form id="reservationStatusUpdate">
			...
			<input type="hidden" id="rest_number" name="rest_number"> <input
				...
				value="/admin/reservation/adminReservationRefundList?page=1&order_by=rest_status&order_sc=DESC">
		</form>
						...
						<c:when test="${not empty adminReservationRefundList}">
							<c:forEach var="reservationRefund"
								...
								<tr data-num="${reservationRefund.rest_number}" data-rest_status="${reservationRefund.rest_status}" id="list_th">
									...
									<td>${reservationRefund.rest_number}</td>
									<td>${reservationRefund.mt_id}</td>
									<td>${reservationRefund.pt_number}</td>
									<td>${reservationRefund.rest_bname}</td>
									<td>${reservationRefund.rest_name}</td>
									<td>${reservationRefund.rest_phone}</td>
									<td>${reservationRefund.rest_status}</td>
									<td>${reservationRefund.rest_total}</td>
									<td>${reservationRefund.rest_exprent}</td>
									<td>${reservationRefund.rest_payoption}</td>
									<td>${reservationRefund.rest_regdate}</td>
									<td>${reservationRefund.rest_count}</td>
								</tr>
							</c:forEach>
						</c:when>
					</c:choose>
